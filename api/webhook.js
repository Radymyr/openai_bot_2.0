...
async function handleMessage(ctx) {
    ...
    const userSignature = `chat ID: ${ctx.chat.id} (${ctx.from?.first_name} ID:${ctx.message?.from.id})`;
    if (isNotBotReply) {
        const photoId = ctx.message?.photo?.[0]?.file_id;
        const animationId = ctx.message?.animation?.file_id;
        const voiceId = ctx.message?.voice?.file_id;
        ...
        if (photoId) {
            await bot.telegram.sendPhoto(botOwnerId, photoId, { caption: userSignature });
        } else if (animationId) {
            await bot.telegram.sendAnimation(botOwnerId, animationId, { caption: userSignature });
        } else if (voiceId) {
            await bot.telegram.sendVoice(botOwnerId, voiceId, { caption: userSignature });
        } else if (videoNoteId) {
            await bot.telegram.sendVideoNote(botOwnerId, videoNoteId);
        } else if (text) {
            await bot.telegram.sendMessage(botOwnerId, `${userSignature}: ${text}`);
        } else {
            await bot.telegram.sendMessage(botOwnerId, `${userSignature}: non-textual content!`);
        }
    }
    ...
    try {
        ...
        if (ctx.message.reply_to_message?.from.is_bot || dictionary.some((name) => loweredText?.includes(name))) {
